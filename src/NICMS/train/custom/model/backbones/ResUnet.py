...
@BACKBONES.register_module()
class ResUnet_enc_lge(nn.Module):
    def __init__(self, in_ch, channels=16, blocks=3):
        super(ResUnet_enc_lge, self).__init__()

        # lge 不对z轴降采样
        self.in_conv = DoubleConv(in_ch, channels, stride=(1,2,2), kernel_size=3)
        self.layer1 = make_res_layer(channels, channels * 2, blocks, stride=(1,2,2))
        self.layer2 = make_res_layer(channels * 2, channels * 4, blocks, stride=(1,2,2))
        self.layer3 = make_res_layer(channels * 4, channels * 8, blocks, stride=(1,2,2))
        self.avgpool = nn.AdaptiveAvgPool3d((1, 1, 1))
    ...
    def forward(self, input):
        ...
        c5 = self.avgpool(c4)
        # z_dim = c4.size(2)
        # c5 = nn.functional.adaptive_avg_pool3d(c4, (z_dim, 1, 1))
...
class CNNTrans_multi(nn.Module):
    def __init__(self, in_ch=1, channels=32, blocks=3, num_classes=5):
        ...
        self.CNN_lge = ResUnet_enc_lge(in_ch=in_ch, channels=channels//2, blocks=blocks)

        self.CNN_4ch.fc = nn.Sequential(nn.Linear(channels * 4, 64))
        self.CNN_sa.fc = nn.Sequential(nn.Linear(channels * 8, 128))
        self.CNN_lge.fc = nn.Sequential(nn.Linear(channels * 4, 64))
        # self.CNN_lge.fc = nn.Sequential(nn.Linear(channels * 8 * 9, 128))
        ...
        self.deep_sup_cls_lge = nn.Linear(64, num_classes)

    def forward(self, vol_4ch, vol_sa, vol_lge):

        x4 = self.CNN_4ch(vol_4ch)
        x4 = self.CNN_4ch.fc(x4)
        xs = self.CNN_sa(vol_sa)
        xs = self.CNN_sa.fc(xs)
        xl = self.CNN_lge(vol_lge)
        xl = self.CNN_lge.fc(xl)

        x = torch.cat([x4, xs, xl], dim=1)  # 256
        deep_sup_cls = self.deep_sup_cls(x)
        deep_sup_cls_4ch = self.deep_sup_cls_4ch(x4)
        deep_sup_cls_sa = self.deep_sup_cls_sa(xs)
        deep_sup_cls_lge = self.deep_sup_cls_lge(xl)

        x = self.transformer(x.unsqueeze(1)).squeeze(1)
        x_cls = self.mlp_head_cls(x)

        return x_cls, deep_sup_cls, deep_sup_cls_4ch, deep_sup_cls_sa, deep_sup_cls_lge

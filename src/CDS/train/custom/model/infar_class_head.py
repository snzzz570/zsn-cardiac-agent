...
class CineClassification_Head(nn.Module):
    ...
    def loss(self, inputs, targets):
        ...
        outs_cls, deep_sup_cls, deep_sup_cls_2ch, deep_sup_cls_4ch, deep_sup_cls_sa = inputs[0], inputs[1], inputs[2], inputs[3], inputs[4]
        loss_cls = self.loss_ce_func(outs_cls, targets)
        loss_cls = loss_cls.mean()
        loss_cls_deep_sup = self.loss_ce_func(deep_sup_cls, targets) * 0.5
        loss_cls_deep_sup = loss_cls_deep_sup.mean()
        loss_cls_deep_sup_2ch = self.loss_ce_func(deep_sup_cls_2ch, targets) * 0.25
        loss_cls_deep_sup_2ch = loss_cls_deep_sup_2ch.mean()
        ...
        return {
                "loss_ce": loss_cls,
                "loss_cls_deep_sup": loss_cls_deep_sup,
                "loss_cls_deep_sup_2ch": loss_cls_deep_sup_2ch,

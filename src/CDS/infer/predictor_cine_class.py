...
class CineClassificationPredictor:
    ...
    def predict(self, vols: list[np.ndarray], num_crops):
        config = self.config.network_cfg

        crop_shapes = [ [80, 192, 192], [80, 192, 192], [288, 144, 144] ]  # target_crop: 2ch, 4ch, sa
        ...
        #         # datas.append(data.cuda().detach())
        #     data_2ch = self._crop_data(vols_torch[0], phy_centers[0], [80, 192, 192])
        #     data_4ch = self._crop_data(vols_torch[1], phy_centers[1], [80, 192, 192])
        #     data_sa = self._crop_data(vols_torch[2], phy_centers[2], [288, 144, 144])
        #     datas.append(data_2ch.cuda().detach())

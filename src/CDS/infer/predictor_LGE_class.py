...
class LGEClassificationPredictor:
    def __init__(self, gpu: int, model: LGEClassificationModel):
        self.gpu = gpu
        self.model = model
        if self.model.config_f is not None:
            if isinstance(self.model.config_f, str):
                with open(self.model.config_f, "r") as config_f:
                    self.config = LGEClassificationConfig(self.model.network_f, yaml.safe_load(config_f),)
            else:
                self.config = LGEClassificationConfig(
    ...
    def build_predictor_from_tar(cls, tar: tarfile.TarFile, gpu: int):
        ...
        return LGEClassificationPredictor(gpu=gpu, model=model_segUrinary_vessel)
    ...
    def predict(self, hu_volume: np.ndarray):
        phy_center = [hu_volume.shape[0]//2, hu_volume.shape[1]//2, hu_volume.shape[2]//2]
        hu_volume = torch.from_numpy(hu_volume.astype(np.float32))[None, None]
        res = self._get_cls_result(hu_volume, phy_center)
        return res
